...
class CropAndExtract():
    ...
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256, still_mode=False):
        ...
        x_full_frames_full = [cv2.cvtColor(
            frame, cv2.COLOR_BGR2RGB) for frame in full_frames]
        ...
        if 'crop' in crop_or_resize.lower():  # default crop
            ...
            x_full_frames_full, crop, quad = self.propress.crop_(
                x_full_frames_full, still=True if 'ext' in crop_or_resize.lower() else False, xsize=512)
        ...
        frames_pil_full = [Image.fromarray(cv2.resize(
            frame, (pic_size, pic_size))) for frame in x_full_frames_full]
        ...
        for frame in frames_pil_full:
            cv2.imwrite(png_path_full, cv2.cvtColor(
                np.array(frame), cv2.COLOR_RGB2BGR))
        ...
        if not os.path.isfile(landmarks_path):
            ...
            lm_full = self.propress.predictor.extract_keypoint(
                frames_pil, landmarks_path)
